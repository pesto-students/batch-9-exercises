...
describe('Promise.each', () => {
  ...
  it('doesnt iterate with an empty array', () => Promise.each([], (_val) => {
    throw new Error();
  }).then((ret) => {
...
describe('Promise.prototype.each', () => {
  ...
  it('doesnt iterate with an empty array', () => Promise.resolve([]).each((_val) => {
    throw new Error();
  }).then((ret) => {
    assert.deepEqual(ret, []);
  }));
...
describe('mapSeries and each', () => {
  it('is mixed', () => Promise.mapSeries([1, 2, 3], (value) => {
    return value * 2;
  }).then((result) => {
